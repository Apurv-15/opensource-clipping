...
def build_config_from_payload(
    ...
    *,
    ...
    cfg = SimpleNamespace(
        ...
        min_clip_duration=int(payload.get("min_clip_duration") or env.get("DEFAULT_MIN_CLIP_DURATION") or 20),
        max_clip_duration=int(payload.get("max_clip_duration") or env.get("DEFAULT_MAX_CLIP_DURATION") or 90),
        hook_source=None,
        ...
        split_trigger=payload.get("split_trigger", "face" if payload.get("use_dynamic_split") else "diarization"),
        use_camera_switch=payload.get("use_camera_switch", False),
        diarization_num_speakers=payload.get("diarization_speakers", "auto"),
        switch_hold_duration=float(payload.get("switch_hold_duration", 2.0)),
        switch_blend_duration=float(payload.get("switch_blend_duration", 0.0)),
        split_zoom=float(payload.get("split_zoom", 1.0)),
        split_v_align=float(payload.get("split_v_align", 0.5)),
        split_auto_zoom=payload.get("split_auto_zoom", False),
        split_max_zoom=float(payload.get("split_max_zoom", 1.5)),
        # Subtitle & Tipografi
        ...
        text_behind_person=payload.get("text_behind_person", False),
        # ASS position values
