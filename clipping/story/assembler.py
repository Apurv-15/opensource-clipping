...
def concat_scenes(
    ...
    """
    if not scene_paths:
        raise ValueError("No scenes provided to concatenate.")
...
def assemble_hook(
    ...
    if not normalized_parts:
        print(f"      ❌ Hook #{cid}: no valid scenes found.")
        return None
...
def assemble_highlight(
    ...
    if not normalized_parts:
        print(f"      ❌ Highlight #{cid}: no valid scenes found.")
        return None
    ...
    print(f"      ✅ highlight_{cid}.mp4 rendered successfully ({total_scenes} scenes, transition: {transition}).")
    return output_path
