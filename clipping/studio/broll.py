...
def download_pexels_broll(query, rasio, output_filename, pexels_api_key):
    ...
    try:
        ...
        print(f"   ⚠️ Pexels API error when searching for '{query}': {e}")
        return False

    if not data.get("videos"):
        print(f"   ⚠️ Pexels found no videos for '{query}'.")
        return False

    available_videos = [v for v in data["videos"] if v["id"] not in USED_PEXELS_IDS]
    if not available_videos:
        print(f"   🔄 B-roll pool exhausted for '{query}', resetting.")
    ...
    if not video_files:
        print(f"   ⚠️ No MP4 file found in video data for '{query}'.")
        return False
    ...
    try:
        ...
        print(f"   ⚠️ Error downloading B-roll '{query}': {e}")
        return False
